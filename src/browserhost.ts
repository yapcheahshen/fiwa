...
export class Fiwa {
	...
	constructor ({sources,onError,onLog}) {
	...
	async instantiate(byteCodes,arg,imports) { //實例化一個wasm模塊，用以載入其他的wasm
		try {
		    ...
		    return wa.instance.exports;  //js 看得到的wasm 函式或變數
		} catch(e) {
	...
	async execute(source,arg){ //組譯並實例化，成功的話就可以bundle
		const A=this.assembler=new Assembler({_mem:this._mem, //雙方可視的memory 區
			imports:{ztype:[1,0],".":[1,0],",":[1,0],"c,":[1,0]}}) //讓wasm 呼叫的host 
		try{
			this.sources.forEach(src=>A.assemble(src));
			A.assemble(source);
